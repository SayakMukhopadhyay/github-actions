...
Generation emits structured diagnostics for GitHub OIDC acquisition, the OpenAI token exchange, the Responses API request, and local response validation. Logs contain only fixed stage and event names plus numeric attempt numbers and HTTP statuses; they never include WIF identifiers, tokens, request or response bodies, repository context, generated text, filesystem paths, or exception messages. Failures use stable stage-specific categories and reasons so authentication, exchange, request, and validation problems can be distinguished without exposing sensitive or untrusted values.
