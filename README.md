...
`SayakMukhopadhyay/github-actions/container-image-inspect@v1` performs a read-only lookup of exactly one normalized `registry/image-repository[/component]:tag` reference. It never builds, pushes, promotes, or tags an image.
...
Generation emits structured diagnostics for GitHub OIDC acquisition, the OpenAI token exchange, the Responses API request, and local response validation. After acquisition, the generator decodes the JWT payload locally and logs only `iss`, `aud`, `sub`, `repository`, `environment`, `job_workflow_ref`, `workflow_ref`, `ref`, and `sha` in a fixed order; absent claims are rendered as `<absent>` and an audience array remains an array. Other logs contain only fixed stage and event names plus numeric attempt numbers and HTTP statuses. Diagnostics never include the encoded JWT, WIF identifiers, authorization headers, request or response bodies, arbitrary claims, repository context supplied to the model, generated text, filesystem paths, exception messages, or stack traces. A malformed JWT or allowlisted claim fails before the OpenAI exchange. Failures use stable stage-specific categories and reasons so authentication, token validity, exchange, request, and validation problems can be distinguished without exposing sensitive or untrusted values.
